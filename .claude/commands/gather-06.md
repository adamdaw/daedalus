...
2. Extract the `## 06 — Runtime View` block. If Status is not `empty`, or if content is populated beyond the skeleton, show the existing content and ask: "Section 06 already has content — would you like to (a) update specific fields, or (b) replace it entirely?"
3. Ask the questions below one topic at a time. Wait for each answer before continuing.
4. Write the structured output back into the `## 06` block of `brief.md`. Update the Status comment to `complete`. Do not modify any other section.
